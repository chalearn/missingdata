function [ cell_h_auroc, h_auroc, h_aulc, h_aupr, AULC, AULCerr, AUPR ] = ...
                            get_overlay_plot( test_r, prec_r, ideal_prec_r, ...
                                        recall_r, ideal_recall_r, num_feats, ...
                                        h_aulc_orig, h_aupr_orig, aulc_avg, ...
                                        sigma_avg, percent_list, position )
...
%   Detailed explanation goes here
    ...
    AULCerr = alc(num_feats, auc_va-sigma_va);
    
    if (isempty(h_aulc_orig))
        h_aulc=overlay_learning_curve(num_feats, auc_va, aulc_avg, ...
                                      sigma_avg, percent_list, position);
    %fprintf('+++ Area under the learnign curve AULC = %5.4f +++\n', AULC);
    else h_aulc=overlay_learning_curve(num_feats, auc_va, aulc_avg, ...
                                       sigma_avg, percent_list, position, ...
                                       h_aulc_orig);
    end
