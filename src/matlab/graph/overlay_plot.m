            ...
            for p=1:length(miss_perc_fold)
                ...
                for s=1:length(miss_sol_fold)
                    ...
                    for i=1:length(miss_methsol_fold)
                        ...
                        [aux1, aux2, h_aulc_array{imput_pos,1}, ...
                         h_aupr_array{imput_pos,1}, AULC, AULCerr, AUPR] = ...
                                get_overlay_plot(valid_r, prec_r, ideal_prec_r, ...
                                            recall_r, ideal_recall_r, num_feats, ...
                                            h_aulc_array{imput_pos,1}, ...
                                            h_aupr_array{imput_pos,1}, ...
                                            aulc_l, sigma_l, percent_list, p);
                        aulc_l = [aulc_l AULC];
                        sigma_l = [sigma_l AULC-AULCerr];
                        for a=1:size(aux1,2);
                            close(aux1{a});
                        end
                        close(aux2);
